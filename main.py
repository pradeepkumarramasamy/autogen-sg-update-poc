...
async def main():
    response = await assistant.run(
        task="Please add port 2 to cft/web-sg.json, commit it with the message 'AutoGen update: open port 2', and make sure to deploy the stack named 'web_sg_stack' using the updated template."
    )
