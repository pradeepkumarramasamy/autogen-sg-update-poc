...
def commit_template(repo_path: str, message: str, template_path: str, stack_name: str | None = None) -> str:
    repo = Repo(repo_path, search_parent_directories=True)
...
commit_template_tool = FunctionTool(
    commit_template,
    description=(
        "Commits and pushes changes to a Git repository, then deploys the stack. "
        "Requires the repo path, commit message, template file path, and stack name."
    )
)
